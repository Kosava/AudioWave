...
class GradientButton(ModernButton):
    ...
    def paintEvent(self, event):
        ...
        if self.icon_text:
            ...
            painter.drawText(0, 0, width, height, Qt.AlignmentFlag.AlignCenter, self.icon_text)
        elif self.text():
            # Draw text
...
class CircularButton(ModernButton):
    ...
    def paintEvent(self, event):
        ...
        painter.setFont(font)
        painter.drawText(0, 0, size, size, Qt.AlignmentFlag.AlignCenter, self.icon_text)
...
class GlassButton(ModernButton):
    ...
    def paintEvent(self, event):
        ...
        # Text/Icon
        text_alpha = int(200 + 55 * self.hover_progress)
        painter.setPen(QColor(255, 255, 255, text_alpha))
        
        if self.icon_text:
            font = QFont("Arial", int(height * 0.4))
        else:
            font = QFont("Arial", 10, QFont.Weight.Medium)
        
        painter.setFont(font)
        text = self.icon_text if self.icon_text else self.text()
...
class ControlButtonPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        if self.style == "circular":
            ...
            self.btn_prev = GlassButton(icon_text="⮜")
            self.btn_prev.setFixedSize(40, 40)
            
            self.btn_play = GlassButton(icon_text="▶")
            self.btn_play.setFixedSize(55, 55)
            
            self.btn_next = GlassButton(icon_text="⮞")
            self.btn_next.setFixedSize(40, 40)
            
            self.btn_stop = GlassButton(icon_text="■")
            self.btn_stop.setFixedSize(40, 40)
        else:  # gradient (default)
            self.btn_prev = GradientButton(icon_text="⮜")
            self.btn_prev.setFixedSize(40, 40)
            
            self.btn_play = GradientButton(icon_text="▶", primary=True)
            self.btn_play.setFixedSize(55, 55)
            
            self.btn_next = GradientButton(icon_text="⮞")
            self.btn_next.setFixedSize(40, 40)
            
            self.btn_stop = GradientButton(icon_text="■")
            self.btn_stop.setFixedSize(40, 40)
    ...
    def set_playing(self, playing):
        ...
        self.btn_play.icon_text = "⏸" if playing else "▶"
        self.btn_play.update()
    
    def set_paused(self):
        """Set to paused state"""
        self.is_playing = False
        self.btn_play.icon_text = "▶"
        self.btn_play.update()
