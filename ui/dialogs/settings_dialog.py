...
# Import Engine Factory za čuvanje preferencije
try:
...
def detect_available_backends():
    ...
    Vrač‡a dict sa statusom svakog backend-a.
    """
    backends = {
        "qt_multimedia": {
            "name": "Qt Multimedia",
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # ✓ NOVO: Učitaj sačuvanu preferenciju audio backend-a
    ...
    def _load_saved_backend_preference(self):
        """
        ✓ NOVO: Učitaj sačuvanu preferenciju audio backend-a i postavi combo box
        """
        try:
            saved_backend = load_engine_preference()
            print(f"[Settings] Loading saved backend preference: {saved_backend}")
            
            # Pronač‘i index u combo box-u po data vrednosti
    ...
    def create_audio_tab(self):
        ...
        for backend_id, info in self.available_backends.items():
            status = "✓" if info["available"] else "Œ"
    ...
    def _on_backend_changed(self, index):
        ...
        if backend_id and backend_id in self.available_backends:
            ...
            eq_text = "✓ Equalizer supported" if info["has_eq"] else "Œ No EQ support"
    ...
    def _on_plugin_toggled(self, plugin_id: str, enabled: bool):
        """Kada se plugin uključi/isključi"""
        if self.plugin_manager:
            if enabled:
    ...
    def create_about_tab(self):
        ...
        info_layout.addRow("Â©ï¸ Copyright:", QLabel(f"{COPYRIGHT_YEAR} {AUTHOR_NAME}"))
    ...
    def apply_settings(self):
        ...
        if current_backend and current_backend != previous_backend:
            ...
            print(f"ℹ️ [Settings] Audio backend unchanged: {current_backend}")
